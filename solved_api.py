...
TODAY = (datetime.datetime.now() - datetime.timedelta(hours=6)).strftime("%Y-%m-%d") # 현재 날짜 (새벽 6시부터 시작하도록)
URL = "https://solved.ac/api/v3/user/show" # solved.ac 홈페이지 주소
HEADERS = {"Content-Type": "application/json"}
USERS = {"unsolved": [], "solved": [], "new_user": [], "none_user": []} # 결과 출력할 때, 사용할 정보들을 담고 있는 사전
TIER = [
    "🖤 newbie",
    "🤎 V",
    "🤎 IV",
...
def get_location(intra_id: str) -> tuple:
    ...
    now_day = datetime.datetime.now() - datetime.timedelta(hours=6) # 익일 새벽 6시를 하루의 끝으로 두고 구한 날짜 -> 6월 5일 0시 0분 0초
